      ...
      {post ? (
        <div className="mt-40 px-30 font-primary">
          ...
          <img
            src={post.image}
            alt=""
            className="w-full rounded-sm border-b-2 border-gray-700"
          />
          <div className="flex justify-between items-center my-3">
            <p className="font-semibold text-gray-400 bg-gray-700 px-5 py-1 rounded-full">
              {post.type}
            </p>
            <button
              onClick={() => addLike(post._id)}
              className="flex items-center gap-1 px-3 py-1 rounded-sm bg-red-300"
            >
              <FcLike /> {post.likes}
            </button>
          </div>
